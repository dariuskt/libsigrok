...
static int sr_ser_tcp_flush(struct sr_serial_dev_inst *serial)
{
sr_err("== TCP FLUSH");

	if (serial->tcp_socket < 0) {
		sr_dbg("Cannot flush unopened port %s.", serial->port);
		return SR_ERR;
	}
...
sr_err("== TCP WRITE nonblocking:%d timeout:%d", nonblocking, timeout_ms);
	...
	ret = send(serial->tcp_socket, buf, count, 0);
